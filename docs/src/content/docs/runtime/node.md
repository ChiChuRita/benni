...
`close()` is safe to call twice, and it is final: once it has run, `redis.session()` and a fresh Pub/Sub subscribe reject with "client is closed" rather than quietly opening a connection nothing will ever close. The same holds for `benni/ioredis` and `benni/bun`.
