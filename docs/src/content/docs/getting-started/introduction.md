...
Benni is a typed **client**, not an ORM. It does not create tables, run migrations, or hide Redis. A Benni schema is a plain TypeScript description of a Redis key family, and raw access is always one call away.
...
Use Benni where schemas help. Use raw Redis where Redis itself is the clearest API.
