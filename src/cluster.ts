...
//
// `benni()` must reference the guard to install it, so anything the root entry
...
// entry keeps the default baseline free of both: `benni/cluster` is only ever
// pulled in by an app that imports it.
//
// Benni checks slot CO-LOCATION, not topology. Routing (MOVED/ASK, per-node
